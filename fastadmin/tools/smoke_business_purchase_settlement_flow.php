...
$cookie = __DIR__ . '/../tmp-fastadmin-cookie-purchase-settlement-smoke.txt';
@unlink($cookie);
...
$stamp = date('YmdHis');
$supplierName = 'smoke-ps-supplier-' . $stamp;
$purchaseTitle = 'smoke-purchase-settlement-' . $stamp;
$settlementTitle = 'smoke-settlement-' . $stamp;
$cleanup = [
    'supplier_id' => 0,
    'purchase_order_id' => 0,
    'approval_id' => 0,
...
try {
    ...
    $purchaseAdd = ajaxPost('business/purchase_order/add', [
        ...
        'row[owner_admin_id]' => (string) $ownerId,
        'row[purchase_content]' => 'smoke purchase settlement flow',
        'row[notes]' => 'smoke-test',
    ], $base, $cookie);
    ...
    $approvalAdd = ajaxPost('business/approval/add', [
        ...
        'row[submit_reason]' => 'smoke purchase settlement approval',
    ], $base, $cookie);
    ...
    $settlementAdd = ajaxPost('business/purchase_settlement/add', [
        ...
        'row[settlement_amount]' => '456.78',
        'row[paid_amount]' => '100.00',
        'row[invoiced_amount]' => '0.00',
        'row[balance_amount]' => '356.78',
    ...
    $paymentDelBlocked = ajaxPost('business/payment_plan/del', ['ids' => (string) $cleanup['payment_plan_id']], $base, $cookie);
    if (($paymentDelBlocked['code'] ?? 0) == 1) {
        throw new RuntimeException('payment plan delete should be blocked by settlement');
    }

    $settlementEdit = ajaxPost('business/purchase_settlement/edit/ids/' . $cleanup['settlement_id'], [
        'row[title]' => $settlementTitle,
        'row[purchase_order_id]' => (string) $cleanup['purchase_order_id'],
        'row[payment_plan_id]' => (string) $cleanup['payment_plan_id'],
        'row[settlement_amount]' => '456.78',
        'row[paid_amount]' => '456.78',
        'row[invoiced_amount]' => '456.78',
        'row[balance_amount]' => '0.00',
        'row[invoice_status]' => 'received',
        'row[invoice_no]' => 'SMOKE-INV-' . $stamp,
        'row[invoiced_at]' => date('Y-m-d'),
        'row[status]' => 'settled',
        'row[owner_admin_id]' => (string) $ownerId,
        'row[settled_at]' => date('Y-m-d H:i:s'),
        'row[notes]' => 'smoke settlement done',
    ], $base, $cookie);
    if (($settlementEdit['code'] ?? 0) != 1) {
        throw new RuntimeException('settlement edit failed: ' . json_encode($settlementEdit, JSON_UNESCAPED_UNICODE));
    }
    ...
    $purchaseOrder = $purchaseRows['rows'][0] ?? null;
    if (!$purchaseOrder || ($purchaseOrder['status'] ?? '') !== 'completed' || (int) ($purchaseOrder['settlement_id'] ?? 0) !== $cleanup['settlement_id']) {
        throw new RuntimeException('purchase order did not complete after settlement: ' . json_encode($purchaseOrder, JSON_UNESCAPED_UNICODE));
    }
    ...
    $purchaseOrder = $purchaseRows['rows'][0] ?? null;
    if (!$purchaseOrder || ($purchaseOrder['status'] ?? '') !== 'processing' || (int) ($purchaseOrder['settlement_id'] ?? 0) !== 0) {
        throw new RuntimeException('purchase order did not roll back to processing after settlement delete: ' . json_encode($purchaseOrder, JSON_UNESCAPED_UNICODE));
    }
    ...
    if (!$purchaseOrder || ($purchaseOrder['status'] ?? '') !== 'approved') {
        throw new RuntimeException('purchase order did not roll back to approved after payment delete: ' . json_encode($purchaseOrder, JSON_UNESCAPED_UNICODE));
    }
    ...
    echo json_encode([
        ...
        'purchase_order_rolled_back_to_processing' => true,
